...
class DB_elo(DB_handler):
    ...
    def get_elo(self, id: int, date: datetime, league: str, starter: bool) -> float:
        ...
        if not query_result and self.get_player_count_per_league(self.session, league) < 50:
    ...
    def average_elo_by_league(self, league: str) -> float:
        ...
        ).filter(Games.league == league).group_by(Games.player_id).subquery()
        ...
        average_elo = self.session.query(func.avg(Elo.elo_value)).join(
            pre_select, (pre_select.c.player_id == Elo.player_id) & (pre_select.c.max_gd == self.Elo.game_date)).scalar()
        return average_elo

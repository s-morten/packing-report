        ...
        [
            dbc.Row(  # Select Columns
                [
                    ...
                    dbc.Col(dmc.MultiSelect(label="Club", id="club-multi-select")),
                    dbc.Col(
                        dbc.Button("Commit", id="commit-button", outline=True, color="dark", size="sm"), align="center"
                    ),
                ],
                justify="around",
            ),
            ...
            dbc.Row(  # Left, right
                [
                    dbc.Col(
                        dbc.Button("<-", id="prev-page-button", outline=True, color="dark", n_clicks=0, size="sm"),
                        width=1,
                        align="end",
                    ),
                    dbc.Col(
                    ...
                    dbc.Col(
                        dbc.Button("->", id="next-page-button", outline=True, color="dark", n_clicks=0, size="sm"),
                        width=1,
                        align="start",
                    ),
                ],
    ...
    def update_table(
        ...
        fig = go.Figure(
            data=[
                go.Table(
                    header=dict(
                        values=result_df.columns, line_color=colors["dark"], fill_color=colors["middle"], align="center"
                    ),
                    cells=dict(
                        values=result_df.T.values,

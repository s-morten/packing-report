...
def get_bookie_oods(home, away, league):
    league_dict = {
        "GER-Bundesliga": "https://www.888sport.com/#/filter/football/germany/bundesliga/",
    ...
    h_odds, d_odds, a_odds = (
        df["full_time_result.1"].values[0],
        df["full_time_result.X"].values[0],
        df["full_time_result.2"].values[0],
    )
    o2p = OddToPercentage()
    ...
    for idx, (_, game_row) in enumerate(next_games_df.iterrows()):
        ...
        data = {
            ...
            "general": {
                ...
                "table_pos_home": set(),
                "table_pos_away": set(),
                "ha_table_pos_home": set(),
                "ha_table_pos_away": set(),
                "form_home_for": set(),
                "form_home_against": set(),
                "form_away_for": set(),
                "form_away_against": set(),
                "ha_form_home_for": set(),
                "ha_form_home_against": set(),
                "ha_form_away_for": set(),
                "ha_form_away_against": set(),
                "elo_home": set(),
                "elo_away": set(),
            },
        }
        starter_home_empty, starter_away_empty = 0, 0
        for player_id, is_home in zip(
            np.concatenate([h_s11, a_s11]),
            np.concatenate([[True for _ in range(11)], [False for _ in range(11)]]),
        ):
            ...
            previous_df = player_df.loc[
                (player_df["starter"] == True) & (player_df["home"] == is_home)
            ]
            ...
            if is_home:
                ...
                is_home = "away"
            data[is_home][is_starter]["xg_for"] += (
                (np.mean((previous_df["xg"] / previous_df["minutes_played"])) * 90)
                if not math.isnan(
                    (np.mean((previous_df["xg"] / previous_df["minutes_played"])) * 90)
                )
                else 0
            )
            data[is_home][is_starter]["xt_all"] += (
                (np.mean((previous_df["xt_all"] / previous_df["minutes_played"])) * 90)
                if not math.isnan(
                    (
                        np.mean((previous_df["xt_all"] / previous_df["minutes_played"]))
                        * 90
                    )
                )
                else 0
            )
            data[is_home][is_starter]["xt_only_pos"] += (
                (
                    np.mean(
                        (previous_df["xt_only_pos"] / previous_df["minutes_played"])
                    )
                    * 90
                )
                if not math.isnan(
            ...
            data[is_home][is_starter]["xd_press"] += (
                (
                    np.mean((previous_df["xd_press"] / previous_df["minutes_played"]))
                    * 90
                )
                if not math.isnan(
                    (
                        np.mean(
                            (previous_df["xd_press"] / previous_df["minutes_played"])
                        )
                        * 90
                    )
                )
            ...
            data[is_home][is_starter]["xd_normal"] += (
                (np.mean((previous_df["xd_def"] / previous_df["minutes_played"])) * 90)
                if not math.isnan(
                    (
                        np.mean((previous_df["xd_def"] / previous_df["minutes_played"]))
                        * 90
                    )
                )
                else 0
            )
            data[is_home][is_starter]["xk"] += (
                (np.mean((previous_df["xk_save"] / previous_df["minutes_played"])) * 90)
                if not math.isnan(
                    (
                        np.mean(
                            (previous_df["xk_save"] / previous_df["minutes_played"])
                        )
                        * 90
                    )
                )
            ...
            data[is_home][is_starter]["gi"] += (
                (np.mean((previous_df["gi"] / previous_df["minutes_played"])) * 90)
                if not math.isnan(
                    (np.mean((previous_df["gi"] / previous_df["minutes_played"])) * 90)
                )
                else 0
            )
            data[is_home][is_starter]["xg_against"] += (
                (
                    np.mean((previous_df["xg_against"] / previous_df["minutes_played"]))
                    * 90
                )
                if not math.isnan(
                    (
                        np.mean(
                            (previous_df["xg_against"] / previous_df["minutes_played"])
                        )
                        * 90
                    )
                )
            ...
            data[is_home][is_starter]["xt_against_all"] += (
                (
                    np.mean(
                        (previous_df["xt_against_all"] / previous_df["minutes_played"])
                    )
                    * 90
                )
                if not math.isnan(
                    (
                        np.mean(
                            (
                                previous_df["xt_against_all"]
                                / previous_df["minutes_played"]
                            )
                        )
                        * 90
                    )
            ...
            data[is_home][is_starter]["xt_against_only_pos"] += (
                (
                    np.mean(
                        (
                            previous_df["xt_against_only_pos"]
                            / previous_df["minutes_played"]
                        )
                    )
                    * 90
            ...
            if home_bool:
                data["general"]["table_pos_home"].add(
                    last_game_df["team_pos"].values[0]
                )
                data["general"]["ha_table_pos_home"].add(
                    last_game_df["team_pos_home_away"].values[0]
                )
                data["general"]["form_home_for"].add(
                    last_game_df["team_form_for"].values[0]
                )
                data["general"]["form_home_against"].add(
                    last_game_df["team_form_against"].values[0]
                )
                data["general"]["ha_form_home_for"].add(
                    last_game_df["team_form_home_away_for"].values[0]
                )
                data["general"]["ha_form_home_against"].add(
                    last_game_df["team_form_home_away_against"].values[0]
                )

            else:
                data["general"]["table_pos_away"].add(
                    last_game_df["opp_position"].values[0]
                )
                data["general"]["ha_table_pos_away"].add(
                    last_game_df["opp_position_home_away"].values[0]
                )
                data["general"]["form_away_for"].add(
                    last_game_df["opp_form_for"].values[0]
                )
                data["general"]["form_away_against"].add(
                    last_game_df["opp_form_against"].values[0]
                )
                data["general"]["ha_form_away_for"].add(
                    last_game_df["opp_form_home_away_for"].values[0]
                )
                data["general"]["ha_form_away_against"].add(
                    last_game_df["opp_form_home_away_against"].values[0]
                )
        ...
        df = pd.DataFrame(
            {
                ...
                "table_pos_home": [
                    most_common(list(data["general"]["table_pos_home"]))
                ],
                "table_pos_away": [
                    most_common(list(data["general"]["table_pos_away"]))
                ],
                "ha_table_pos_home": [
                    most_common(list(data["general"]["ha_table_pos_home"]))
                ],
                "ha_table_pos_away": [
                    most_common(list(data["general"]["ha_table_pos_away"]))
                ],
                "form_home_for": [most_common(list(data["general"]["form_home_for"]))],
                "form_home_against": [
                    most_common(list(data["general"]["form_home_against"]))
                ],
                "form_away_for": [most_common(list(data["general"]["form_away_for"]))],
                "form_away_against": [
                    most_common(list(data["general"]["form_away_against"]))
                ],
                "ha_form_home_for": [
                    most_common(list(data["general"]["ha_form_home_for"]))
                ],
                "ha_form_home_against": [
                    most_common(list(data["general"]["ha_form_home_against"]))
                ],
                "ha_form_away_for": [
                    most_common(list(data["general"]["ha_form_away_for"]))
                ],
                "ha_form_away_against": [
                    most_common(list(data["general"]["ha_form_away_against"]))
                ],
                ...
                "home_xt_only_pos_against": [
                    data["home"]["starter"]["xt_against_only_pos"]
                ],
                ...
                "home_sub_xt_only_pos_against": [
                    data["home"]["subs"]["xt_against_only_pos"]
                ],
                ...
                "away_xt_only_pos_against": [
                    data["away"]["starter"]["xt_against_only_pos"]
                ],
                ...
                "away_sub_xt_only_pos_against": [
                    data["away"]["subs"]["xt_against_only_pos"]
                ],
            }
        )
...
def calc_odds():
    ...
    next_hour = time_now + timedelta(weeks=1)

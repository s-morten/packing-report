...
print(f"{datetime.now().strftime('%d/%m/%Y %H:%M:%S')} Number of Leagues: {len(league_list)}")
...
print(f"{datetime.now().strftime('%d/%m/%Y %H:%M:%S')} Number of Games: {len(game_list)}")
time.sleep(7200)
...
while game_list:
    ...
    if len(game_list) > num_requests:
        ...
        print(f"{datetime.now().strftime('%d/%m/%Y %H:%M:%S')} Number of new Requests: {len(game_sub_list)}")
        print(f"{datetime.now().strftime('%d/%m/%Y %H:%M:%S')} Number of Requests left: {len(game_list)}")
    ...
    for game in game_sub_list:
        ...
        if data['data']['lineup']['data']:
            print(f"{datetime.now().strftime('%d/%m/%Y %H:%M:%S')} Lineup is empty: skip {game}")
            continue
        if data['data']['corners']['data']:
            print(f"{datetime.now().strftime('%d/%m/%Y %H:%M:%S')} Corners is empty: skip {game}")
            continue
        ...
        for x in data["data"]["corners"]["data"]:
            if(x['comment'][0] == 'R'): continue # skip "Race to Nth Corner events"
            team_id = x['team_id'] 
            corner_min = x['minute']
            cur.execute(f"INSERT INTO game (game_id, corner_min, team_id, date) VALUES ({game},{corner_min},{team_id},{date})")
            print(f"{datetime.now().strftime('%d/%m/%Y %H:%M:%S')} Added Corners to Game: {game}")
        ...
        for x in data["data"]["lineup"]["data"]:
            ...
            print(f"{datetime.now().strftime('%d/%m/%Y %H:%M:%S')} Added Lineup for Game: {game}")
        ...
        for x in data["data"]["substitutions"]["data"]:
            ...
            min_off = 90
            cur.execute(f"INSERT INTO lineup (game_id, player_id, home, min_on, min_off, team_id) VALUES ({game},{player_id},{home},{min_on},{min_off},{x['team_id']})")
            print(f"{datetime.now().strftime('%d/%m/%Y %H:%M:%S')} Added Subs for Game: {game}")

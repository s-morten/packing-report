...
class DB_games():
    ...
    def insert_game(self, game_id: int, player_id: int, minutes: int, starter: bool, 
        ...
        game = Games(game_id=int(game_id), player_id=int(player_id), minutes=int(minutes), starter=int(starter), opposition_team_id=int(opposition_team_id),
                            ...
                            version=float(version), home=int(home), game_minutes=int(game_minutes), valid = int(valid))
        self.session.add(game)
        self.session.commit()

    def get_all_games(self, version: float, last: int = -1):
        if last == -1:
            query_result = self.session.query(Games.game_id, Games.minutes, Games.elo, Games.opposition_elo, Games.result, Games.home, Games.game_minutes).filter(Games.version == version, Games.valid == 1).all()
        else:
    ...
    def insert_games_batch(self, games: list[list]):
        converted_games = [Games(game_id=int(game[0]), player_id=int(game[1]), minutes=int(game[2]), starter=int(game[3]), opposition_team_id=int(game[4]),
                            ...
                            team_id=int(game[9]), expected_game_result_lower=float(game[10]), 
                            expected_game_result_upper=float(game[11]), league=str(game[12]), 
                            version=float(game[13]), home=int(game[14]), game_minutes=int(game[15]), valid = int(game[16])) for game in games]
